...
export default async function sendTemplateEmail({
  ...
  const htmlEmailContents = await Helpers.utils.renderView(
    emailTemplatePath,
    templateData,
    layout,
  );
  ...
  if (config.env === 'test' || isToAddressConsideredFake) {
    logger.debug(
    ...
    if (
      ...
      ` +
          ((): string => {
            ...
            if (!emailConfig.host) {
              problems.push(
                " • SMTP host is missing from this app's configuration (`Config.email.host`)",
              );
            }
            if (!emailConfig.port) {
              problems.push(
                " • SMTP port is missing from this app's configuration (`Config.email.port`)",
              );
            }
            if (!emailConfig.auth.user) {
              problems.push(
                " • SMTP user is missing from this app's configuration (`Config.email.auth.user`)",
              );
            }
            if (!emailConfig.auth.pass) {
              problems.push(
                " • SMTP password is missing from this app's configuration (`Config.email.auth.pass`)",
              );
            }
